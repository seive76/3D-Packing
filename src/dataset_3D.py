...
class ProductDataset3D:
    ...
    def _gen_products(self):
        ...
        dw_ratios = np.random.normal(loc=0.695, scale=0.218, size=(self.num_products, 1))
        hw_ratios = np.random.lognormal(mean=0.654, sigma=0.453, size=(self.num_products, 1))
        volumes = np.random.lognormal(mean=2.568, sigma=1.205, size=(self.num_products, 1))
        ...
        df = pd.DataFrame(dims, columns=["width", 
                                         "depth", 
                                         "height", 
                                         ])
        df["volume"] = (df.width * df.height * df.depth)/1000. # litter
    ...
    def get_dummy_order(self, ordered_products, dim="height"):
        ...
        order["volume"] = (order.width * order.height * order.depth) / 1000000. # litter
