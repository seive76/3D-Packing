...
class Dimension:
    ...
    def __str__(self):
        return (
            f"Dimension(width={self.width}, height={self.height}, "
...
class Coordinate:
    ...
    def from_blb_to_vertices(self, dims):
        ...
        brb = Coordinate(self.x + dims.width, self.y)#, self.z)
        #brf = Coordinate(self.x + dims.width, self.y + dims.height)#, self.z)
        tlb = Coordinate(self.x, self.y + dims.height)#, self.z + dims.height)
        #tlf = Coordinate(self.x + dims.width, self.y, self.z + dims.height)
        trb = Coordinate(self.x + dims.width, self.y + dims.height)#, self.z + dims.height)
        #trf = Coordinate(self.x + dims.width, self.y + dims.depth, self.z + dims.height)
        return [blb, brb, tlb, trb]
...
class Vertices:
    ...
    def __init__(self, blb, dims):
        ...
        self.brb = Coordinate(self.blb.x, self.blb.y + self.dims.height)#depth, self.blb.z)
        #self.brf = Coordinate(
        #    self.blb.x + self.dims.width, self.blb.y + self.dims.depth, self.blb.z
        #)

        # Top left back and front
        self.tlb = Coordinate(self.blb.x, self.blb.y + self.dims.height)#, self.blb.z + self.dims.height)
        #self.tlf = Coordinate(
        ...
        self.trb = Coordinate(
            self.blb.x + self.dims.width, self.blb.y + self.dims.height#, self.blb.z + self.dims.height
        )
        #self.trf = Coordinate(
    ...
    def to_faces(self):
        ...
        (1 (6 different in 3D) faces for one cuboid)
        """
        return np.array(
            #[
                [
...
def do_overlap(a, b):
    ...
    dy = min(a.y.item() + a.height.item(), b.y.item() + b.height.item()) - max(a.y.item(), b.y.item()) # depth in 3D
    if (dx > 0) and (dy > 0):
        return True
    return False
...
def get_pallet_plot(pallet_dims):
    ...
    ax.set_ylabel("y")
    #ax.set_zlabel("z")
    ax.text(0, 0, 0, "origin", size=10, color="k")#, zorder=1
    #ax.view_init(azim=60)
    ax.set_xlim(0, pallet_dims.width)
    ax.set_ylim(0, pallet_dims.height)
    #ax.set_zlim3d(0, pallet_dims.height)
    return ax


def plot_product(blb, ax, item_id, coords, dims):
    """
    Add product to given axis
    """
    vertices = Vertices(coords, dims)
    ax.scatter(vertices.get_xs(), vertices.get_ys())#, vertices.get_zs())
