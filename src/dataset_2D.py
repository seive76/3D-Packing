...
class ProductDataset2D:
    ...
    def _gen_products(self):
        ...
        hw_ratios = np.random.normal(loc=0.654, scale=0.453, size=(self.num_products, 1))
        volumes = np.random.lognormal(mean=2.568, sigma=1.205, size=(self.num_products, 1)) #* 1e6

        # Generate each dimension separately
        widths = np.clip(
            np.power(volumes / hw_ratios, 1),
            self.min_width,
            self.max_width,
        )
        ...
        df["volume"] = (df.width * df.height) / 100 # m3
                        #df.depth *\
    ...
    def get_dummy_order(self, ordered_products, dim="height"):
        ...
        order["volume"] = (order.width * order.height) / 100 #m3
